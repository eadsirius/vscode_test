...
class psearch extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->helper(array('file','form', 'url','search', 'office'));
	...
	public function index()
	{
		...
		layout('/member/pwSearch',$data,'login');
	...
	function emailSend() 
	{
		...
		$id_chk = $this->m_member->id_check($user_id);
		$chk = $this->m_member->get_email_id_check($user_id,$email);
		
		 if(empty($id_chk)){
			alert(get_msg($this ->lang, '회원 ID가 존재하지 않습니다.'));
		 }
		
		if(empty($chk)){
			alert(get_msg($this ->lang, '이메일 주소가 존재하지 않습니다.'));
		}
		...
		$this->mail_send($user_id,$email,$authcode);	
		
		$input['member_id'] 	= $user_id;
		$db_input['password']	=	$authcode;
		
		$this->m_member->user_update($db_input,$input);
		
		alert(get_msg('', '변경된 비밀번호가 이메일로 전송되었습니다.'), "member/login");
	}
	...
	function resetPw()
	{
		...
		{
			layout('/member/pwReset',$data,'none');
			...
			alert("Create Password", "member/login");	
		}
	}
	...
	function emailResult()
	{
		...
		layout('/member/pwSend',$data,'none');
	}
	...
	function smsSend() 
	{
		...
		$mobile = "82" .$mobile[0].@$mobile[1].@$mobile[2];
		...
		$smslib = new SmsLib();
		$response = $smslib->send($mobile, "Your uceasset verification code is: {$authcode}");

		//echo $response;
		redirect('/member/psearch/presult/' .$mobile1 .'/' .$authcode);
	...
	function presult()
	{
		...
		$data['mobile'] = $this->uri->segment(4,0);
		$data['revsns'] = $this->uri->segment(5,0);
		
		//---------------------------------------------------------------------------------//
		$this->form_validation->set_rules('sns', 'sns', 'required');
		$this->form_validation->set_rules('new_password', 'new_password', 'required');
		$this->form_validation->set_rules('new_password_confirm', 'new_password_confirm', 'required');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			layout('/member/pwResult',$data,'login');

		} else {
			
			$revsns = $this->input->post('revsns');
			$mobile = $this->input->post('mobile');
			
			$member = $this->m_member->get_mobile($mobile);
			if (!$member) {
				alert("$mobile 존재하지 않는 휴대폰 번호입니다");	
			}
			else
			{		
				...
				if($sns == $revsns){
					...
					alert("비밀번호 생성이 완료되었습니다. 다시 로그인해주세요", "member/login/out");
	...
	public function mail_send($user_id,$email,$authcode) 
	{
							...
							<div style='Margin-left: 20px;Margin-right: 20px;'>
								<div style='mso-line-height-rule: exactly;mso-text-raise: 4px;'>
									<h1 style='Margin-top: 0;Margin-bottom: 0;font-style: normal;font-weight: normal;color: #565656;font-size: 30px;line-height: 38px;text-align: center;'>
										<strong>비밀번호 변경 완료!</strong></h1>
									<p style='Margin-top: 20px;Margin-bottom: 0;'>&nbsp;<br>
										[".$user_id."]회원님 비밀번호가 변경되었습니다.
										</p>
									<p style='Margin-top: 20px;Margin-bottom: 20px;'>변경된 비밀번호입니다.</p>
      							</div>
    						</div>

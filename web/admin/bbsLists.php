...
-->
					<a href="/admin/bbs/add/<?=$bbs_name?>"><button type="button" class="btn btn-dark d-none d-lg-block m-l-15"><i
								class="fa fa-plus-circle"></i> 게시판 글쓰기</button></a>
				</div>
			</div>
		</div>
		<!-- ============================================================== -->
		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					<div class="card-body">
						<h4 class="card-title"><?=$title?> 게시판관리</h4>
						<h6 class="card-subtitle">각 게시판 리스트를 확인하실 수 있습니다.</h6>

						<div class="row">
							<div class="col-sm-12 col-md-6">
								<div class="dataTables_length">
									<form name='fsearch' method='post' action="<?=SEARCH_URL?>">
										<select class="custom-select col-12" id="inlineFormCustomSelect" style="width: 100px; height: 28px; appearance: auto;"
											id='st' name='st'>
											<option value='subject' <?if($st=='subject' ){?>selected
												<?}?>>제목</option>
											<option value='contents' <?if($st=='contents' ){?>selected
												<?}?>>내용</option>
										...
										<button type="button" class="btn1 btn-primary" style="height: 28px;background:#2962FF;border-color:#2962FF" onclick="location.href='/admin/bbs/lists/notice'">초기화</button>
									</form>
								</div>
							</div>
							<div class="col-sm-12 col-md-6">
								<div class="dataTables_filter">
									...
									<select name="list" class="form-control form-control-sm" onchange="location.href=this.value">
										...
										<option value="/admin/config/page/10/group" <?if($list_per==10){?>selected
											<?}?>>10</option>
										<option value="/admin/config/page/25/group" <?if($list_per==25){?>selected
											<?}?>>25</option>
										<option value="/admin/config/page/50/group" <?if($list_per==50){?>selected
											<?}?>>50</option>
										<option value="/admin/config/page/100/group" <?if($list_per==100){?>selected
											<?}?>>100</option>
									</select>
								</div>
							</div>
						...
						<table id="mainTable" class="table editable-table table-bordered table-striped m-b-0">
							...
							<tbody>
    							...
    							?>
								<tr>
									<td class="txt_right"><?=$i?></td>
									<td class="txt_left"><?=$row->category?></td>
									<td class="txt_left"><?=$row->subject?></td>
									<td class="txt_left"><?=$row->member_id?></td>
									<td class="txt_center"><?=date("Y-m-d H:i:s",strtotime($row->regdate." +9 hours"))?></td>
									<td>
										<a href="/admin/bbs/writer/<?=$row->bbs_no?>/<?=$bbs_name?>"
											class="btn btn-secondary footable-edit">
											<span class="fas fa-pencil-alt" aria-hidden="true"></span></a>

										<a href="/admin/bbs/delete/<?=$row->bbs_no?>/<?=$table?>" class="btn btn-secondary footable-delete">

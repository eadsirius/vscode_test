                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <h6 class="card-subtitle">매수신청을 파악하실 수 있습니다.</h6>
    							...
    							?>
                                    <tr>
										...
										<td><?=number_format($row->saved_point)?></td>
										<td><?=$row->regdate?></td>
										...
										<td>											
											<?if($kind != 'no'){?>
											<?if($row->kind == 'apply'){?>
	                                        <a href="/admin/point/apply/<?=$row->point_no?>"> <button type="button" class="btn waves-effect waves-light btn-rounded btn-xs btn-info">승인하기</button></a>
											<?}else{?>
	                                        <a href="/admin/point/applyout/<?=$row->point_no?>"> <button type="button" class="btn waves-effect waves-light btn-rounded btn-xs btn-danger">승인취소</button></a>
											<?}?><?}?>
										</td>		                                        
										<?}?>
										...
										<td>
	                                    <?if($this->session->userdata('level') > 9){?>
	                                        <a href="/admin/point/pointRev_write/<?=$row->point_no?>/<?=$table?>" class="btn btn-secondary footable-edit">
		                                        <span class="fas fa-pencil-alt" aria-hidden="true"></span></a>		                                        
										<?}?>
	                                    <?if($this->session->userdata('level') > 9){?>
											<a href="/admin/point/delete/<?=$row->point_no?>/<?=$table?>" class="btn btn-secondary footable-delete">
